...
$(CONVERSION_VERIFIER): $(TOOLS_DIR)/go.mod # Build conversion-verifier from tools folder.
	cd $(TOOLS_DIR); go build -tags=tools -o $(BIN_DIR)/$(CONVERSION_VERIFIER_BIN) github.com/eitco/cluster-api-addon-provider-cdk8s/hack/tools/conversion-verifier # Changed to cdk8s
...
$(RUNTIME_OPENAPI_GEN): $(TOOLS_DIR)/go.mod # Build openapi-gen from tools folder.
	cd $(TOOLS_DIR); go build -tags=tools -o $(BIN_DIR)/$(RUNTIME_OPENAPI_GEN_BIN) github.com/eitco/cluster-api-addon-provider-cdk8s/hack/tools/runtime-openapi-gen # Changed to cdk8s
...
$(TILT_PREPARE): $(TOOLS_DIR)/go.mod # Build tilt-prepare from tools folder.
	cd $(TOOLS_DIR); go build -tags=tools -o $(BIN_DIR)/tilt-prepare github.com/eitco/cluster-api-addon-provider-cdk8s/hack/tools/tilt-prepare # Changed to cdk8s
